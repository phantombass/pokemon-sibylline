...
#		opps = battler.eachOpposing
		...
		battler.moves do |m|
			...
			opps do |o|
				if PurifyChamberSet.isSuperEffective(m.type,o.types[0])
					move_super = true
				end
				if o.type2
					if PurifyChamberSet.isSuperEffective(m.type,o.types[1])
						move_super = true
					end
				end
    ...
    if !shouldSwitch && battler.turnCount>-1 && skill>=PBTrainerAI.highSkill
      target = battler.pbDirectOpposing(true)
				battler_eff1 = GameData::Type.get(target.types[0]).effectiveness(battler.types[0]) * (target.types[1] != nil ? GameData::Type.get(target.types[1]).effectiveness(battler.types[0]) : 1)
				if battler.types.length > 1
					battler_eff2 = GameData::Type.get(target.types[0]).effectiveness(battler.types[0]) * (target.types[1] != nil ? GameData::Type.get(target.types[1]).effectiveness(battler.types[1]) : 1)
				end
				target_eff1 = GameData::Type.get(battler.types[0]).effectiveness(target.types[0]) * (battler.types[1] != nil ? GameData::Type.get(battler.types[1]).effectiveness(target.types[0]) : 1)
				if target.types.length > 1
					target_eff2 = GameData::Type.get(battler.types[0]).effectiveness(target.types[1]) * (battler.types[1] != nil ? GameData::Type.get(battler.types[1]).effectiveness(target.types[1]) : 1)
				end
				type1Battler = battler_eff1
				type2Battler = battler_eff2
				type1Target = target_eff1
				type2Target = target_eff2
				battler_SE = 8
				battler_2SE = 16
				# 1 = NVE
				# 2 = Neutral
...
class Battle::AI
	...
	def pbGetMoveScore(move,user,target,skill=100)
		...
		if skill>=PBTrainerAI.mediumSkill
         ...
         user.hasActiveAbility?(:GORILLATACTICS) || user.hasActiveAbility?(:FORESTSSECRETS)
