class Battle::AI
  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    when "RaiseUserAttack1"
      if move.statusMove?
        if user.statStageAtMax?(:ATTACK)
          ...
          score += 30 if [:PHYSICALBREAKER,:SETUPSWEEPER,:WINCON].include?(user.role)
          if skill >= PBTrainerAI.mediumSkill
    ...
    when "RaiseUserSpeed1"
      ...
      score += 30 if [:SPECIALBREAKER,:PHYSICALBREAKER,:SETUPSWEEPER,:WINCON].include?(user.role)
    #---------------------------------------------------------------------------
    ...
    when "ProtectUser"
      if user.effects[PBEffects::ProtectRate] > 1 ||
        ...
        score += 70 if target.status == :POISON
        score += 30 if [:TOXICSTALLER,:PHYSICALWALL,:SPECIALWALL,:STALLBREAKER].include?(user.role) && target.status == :POISON
    ...
    when "RemoveUserBindingAndEntryHazards"
      ...
      if @battle.pbAbleNonActiveCount(user.idxOwnSide)>0
        ...
        if user.pbOwnSide.effects[PBEffects::StealthRock] || user.pbOwnSide.effects[PBEffects::ToxicSpikes]>0 || user.pbOwnSide.effects[PBEffects::Spikes]>0 || user.pbOwnSide.effects[PBEffects::StickyWeb]
