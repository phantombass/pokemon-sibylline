  ...
  class Battler_AI_Learning
    ...
    def get_item_score
      ...
      scores = items.map do |item|
        if item != :REVIVE && item != :MAXREVIVE
          ...
          next [0, item] if !Battle::ItemEffects.triggerCanUseInBattle(item, @battler.pokemon, @battler, nil, false, @battle, nil, false)
        end
    ...
    def get_move_score(target, move)
      ...
      if move.statusMove?
        ...
        score = [:PHYSICALWALL,:SPECIALWALL,:PIVOT,:SPEEDCONTROL,:STALLBREAKER,:CLERIC].include?(self.role) ? 100 : 30
    ...
    def estimate_hp_difference_at_end_of_round
      ...
      if takes_indirect_damage?
        ...
        if $cinders > 0 && @battler.affectedByCinders?
          lost += @battler.totalhp / 16.0
        end
        if @battler.effectiveField == :Lava && @battler.takesLavaDamage?
          lost += @battler.totalhp / 16.0
        end
        # Sleep + Nightmare
